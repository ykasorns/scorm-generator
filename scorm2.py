...
if 'settings' not in st.session_state:
    st.session_state['settings'] = {"title": "Security Awareness Training", "color": "#D2836C", "pass": 80}
...
with st.sidebar:
    ...
    with col_save:
        project_data = {
            ...
            "timeline": [{k: v for k, v in item.items() if k != 'file'} for item in st.session_state['timeline']]
        }
        json_str = json.dumps(project_data, ensure_ascii=False, indent=2)
        st.download_button("💾 Save Project", json_str, file_name="course_project.json", mime="application/json")
    
    with col_load:
        uploaded_json = st.file_uploader("📂 Load Project", type=['json'], label_visibility="collapsed")
    ...
    with st.expander("⚙️ Course Settings", expanded=True):
        ...
        logo_file = st.file_uploader("Logo (PNG/JPG)", type=['png', 'jpg'])
...
if not st.session_state['timeline']:
    ...
    # --- EXPORT ---
    files_missing = any(item['type'] in ['video', 'pdf', 'image'] and 'file' not in item for item in st.session_state['timeline'])
        ...
        if st.button("🚀 Export SCORM Package (.zip)", type="primary", use_container_width=True):
            with st.spinner("Packaging..."):
                ...
                with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                    ...
                    if logo_file:
                        zf.writestr(logo_file.name, logo_file.getvalue())
                        logo_html = f'<img src="{logo_file.name}">'
                        logo_html_large = f'<div style="background-color:{st.session_state["settings"]["color"]} !important; -webkit-print-color-adjust: exact; padding:20px; border-radius:10px; display:inline-block; margin-bottom:20px;"><img src="{logo_file.name}" style="height:60px; display:block;"></div>'
                        resource_files_xml += f'<file href="{logo_file.name}"/>'

                    for idx, item in enumerate(st.session_state['timeline']):
                        data_obj = {"type": item['type']}
                        if 'title' in item: data_obj['title'] = item['title']
                        if 'content' in item: data_obj['content'] = item['content']
                        
                        if 'file' in item:
                            clean_name = f"res_{idx}_{item['filename']}"
                            zf.writestr(clean_name, item['file'].getvalue())
                            data_obj['src'] = clean_name
                            resource_files_xml += f'<file href="{clean_name}"/>'
                        
                        if item['type'] == 'quiz':
                            data_obj.update({"question": item['question'], "quizType": item['quizType'], "options": item['options'], "correct": item['correct']})
                        
                        js_course_data.append(data_obj)

                    html_content = HTML_TEMPLATE.format(
                        course_title=st.session_state['settings']['title'],
                        theme_color=st.session_state['settings']['color'],
                        passing_score=st.session_state['settings']['pass'],
                        logo_html=logo_html,
                        logo_html_large=logo_html_large,
                        course_data_json=json.dumps(js_course_data, ensure_ascii=False),
                        has_quiz="true" if any(x['type'] == 'quiz' for x in st.session_state['timeline']) else "false"
                    )
                    zf.writestr("index.html", html_content)
                    
                    xml_content = MANIFEST_TEMPLATE.format(id=unique_id, title=st.session_state['settings']['title'], resource_files=resource_files_xml)
                    zf.writestr("imsmanifest.xml", xml_content)

                st.success("Export Successful!")
                st.download_button("⬇️ Download ZIP", data=zip_buffer.getvalue(), file_name=f"Course_{unique_id}.zip", mime="application/zip")
